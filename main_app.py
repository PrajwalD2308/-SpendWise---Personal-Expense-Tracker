...
def open_main_app(root, conn, cursor, username):
    ...
    ttk.Button(add_frame, text="Add Expense", command=lambda: add_expense(
        cursor, conn, username, amount_entry, category_var, desc_entry, date_entry,
        lambda: clear_fields(amount_entry, desc_entry, date_entry, category_var),
        lambda: load_expenses(cursor, tree, username)
    )).grid(row=4, columnspan=2, pady=10)

    ttk.Button(root, text="Show Pie Chart", command=lambda: show_pie_chart(cursor, username)).pack(pady=10)
    load_expenses(cursor, tree, username)
